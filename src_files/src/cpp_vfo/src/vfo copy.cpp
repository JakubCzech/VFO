...
public:
  ...
  void VFO(double x, double y, double theta, double F, double Fx, double Fy, double Fxx, double Fyy, double Fxy);
...
void vfo::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  VFO_point(166.0, 6.0, 1.0);
  BSP20();
  publish_cmd_vel();
}

void vfo::VFO(double F, double Fx, double Fy, double Fxx, double Fyy, double Fxy)
    {
         // ||∇F (p)||
...
std::array<double, 2> get_function(double x, double y)
{
...
// Prosta
  ...
  std::array<double, 6> result = {F, Fx, Fy, Fxx, Fyy, Fxy};
  return result;
}
